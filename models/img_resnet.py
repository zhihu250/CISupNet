...
class ResNet50(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.uncloth_dim = config.MODEL.FEATURE_DIM // 8
        self.contour_dim = config.MODEL.FEATURE_DIM // 4
        
        self.uncloth_net = GEN(in_feat_dim = self.uncloth_dim, out_img_dim=1, config = config)
        self.contour_net = GEN(in_feat_dim = self.contour_dim, out_img_dim=1, config = config)
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        f = self.bn(x)
        f_unclo = base_f[:, 0:self.uncloth_dim, :, :]
        f_cont = base_f[:, self.uncloth_dim:self.uncloth_dim+self.contour_dim, :, :]
        unclo_img = self.uncloth_net(f_unclo)
        cont_img  = self.contour_net(f_cont)
        return base_f, f, unclo_img, cont_img