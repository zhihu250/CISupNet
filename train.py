...
# 正常训练一个resnet50以及一个classifier 没有klloss
def train_cal(config, epoch, model, model2, fuse, classifier, clothes_classifier, clothes_classifier2,criterion_cla, criterion_pair, 
    criterion_clothes, criterion_adv, optimizer, optimizer2, optimizer_cc, trainloader, pid2clothes, kl, recon_cloth, recon_uncloth, recon_contour):
    ...
    for batch_idx, (imgs, pids, camids, clothes_ids, cloth, _cloth, contour) in enumerate(trainloader):
        ...
        # Forward
        pri_feat, features, uncloth_img, contour_img = model(imgs) # torch.size([32,4096])
        ...
        # out_id_clothes = clothes_classifier3(features_fuse - features)
        ...
        # Update the clothes discriminator只更新这个clothes_classifier
        ...
        cont_loss = recon_contour(contour_img, contour)
        unclo_loss = recon_uncloth(uncloth_img, _cloth)
        
        # 更新衣服分支
        rec_loss = cont_loss + unclo_loss
        ...
        if epoch >= config.TRAIN.START_EPOCH_ADV:
            loss = cla_loss + adv_loss + config.LOSS.PAIR_LOSS_WEIGHT * pair_loss + 0.7 * rec_loss
        else:
            loss = cla_loss + config.LOSS.PAIR_LOSS_WEIGHT * pair_loss + 0.7 * rec_loss
    ...
    logger.info('Epoch{0} '
                  ...
                  'Clo2Acc:{clo2_acc.avg:.2%} '
                  'Cloth_gen_loss:{cloth_gen.avg:.2%}'
                  'unclo_gen_loss:{unclo_loss.avg:.4f} '
                  'cont_gen_loss:{cont_loss.avg:.4f} '.format(
                   ...
                   cloth_gen=batch_rec_loss,
                   unclo_loss = batch_unclo_rec_loss, cont_loss = batch_cont_rec_loss))
